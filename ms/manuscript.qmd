...
```{r}
ggplot(arth_meter, aes(site, llz, color = site)) +
    # geom_boxplot() +
    geom_jitter(width = 0.1) +
    scale_color_viridis_d(option = "magma")

sum(arth_meter$site == "LA")


unique(arth[, c("site", "tree")])
```
...
### Compare deviations to state variables

```{r}
state_dev<-group_by(arth,site,tree) |> 
    summarise(
        nspp=n_distinct(species_code),
        n=sum(abundance),
        tmet=sum(metabolic_rate)
    ) |>  
    ungroup() |> 
    full_join(arth_meter)


ggplot(state_dev, aes(nspp, llz, color = site)) +
    geom_point() +
    scale_color_viridis_d(option = "magma", direction = -1)

ggplot(state_dev, aes(n, llz, color = site)) +
    geom_point() +
    scale_color_viridis_d(option = "magma", direction = -1)

ggplot(state_dev, aes(tmet, llz, color = site)) +
    geom_point() +
    scale_color_viridis_d(option = "magma", direction = -1)
