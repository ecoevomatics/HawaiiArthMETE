...
# filter(arth_mete, site %in% c("VO", "KH") & (z2_ipd > 11 | z2_ipd < 0.2))
...
#           .by = c(site, tree)) |> 
#     full_join(arth_mete) |> 
#     ggplot(aes(z2_sad, z2_ipd, color = site_name, size = n)) +
#     geom_point() +
#     this_ms_look(log_x = TRUE, log_y = TRUE)
# 
# ggplot(arth_mete, aes(z2_sad, z2_ipd, color = site_name)) +
...
# remove this and subseqent chunks
r <- readLines(r_temp) 
r <- r[-(grep("label: re-run", r):length(r))]
```
